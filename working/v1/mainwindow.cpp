...
{
    bool ok;
    QString key = QInputDialog::getText(this, "Govee API Key", "Enter API key:",
...
void MainWindow::loadDevices()
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]{
        ...
        if (reply->error() != QNetworkReply::NoError || doc.object()["code"].toInt() != 200) {
            QMessageBox::critical(this, "Error", "Invalid API key or network");
            return;
        }
...
QWidget* MainWindow::createGroupControl(const QVector<QJsonObject> &devices, const QString &title)
{
    ...
    auto updatePowerText = [groupPower, allOn, someOn]() mutable {
        if (someOn) groupPower->setText("Partial On");
        else groupPower->setText(allOn ? "Turn Group Off" : "Turn Group On");
    };
    updatePowerText();

    connect(groupPower, &QPushButton::toggled, [=](bool on) mutable {
        allOn = on;
        someOn = false;
        updatePowerText();
